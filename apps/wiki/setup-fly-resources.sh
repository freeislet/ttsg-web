...
echo -e "${GREEN}TTSG Wiki - Fly.io 리소스 생성 스크립트${NC}"
echo "========================================"
...
if flyctl postgres create --name ttsg-wiki-db --region nrt --vm-size shared-cpu-1x --initial-cluster-size 1 --volume-size 1; then
  echo -e "${GREEN}PostgreSQL 데이터베이스 생성 완료!${NC}"
else
  echo -e "${RED}PostgreSQL 데이터베이스 생성 실패 또는 이미 존재합니다.${NC}"
  echo "기존 데이터베이스 연결 정보를 확인하세요: flyctl postgres show ttsg-wiki-db"
fi

# Redis 인스턴스 생성
echo -e "\n${YELLOW}Redis 인스턴스 생성 중...${NC}"
if flyctl redis create --name ttsg-wiki-redis --region nrt --vm-size micro; then
  echo -e "${GREEN}Redis 인스턴스 생성 완료!${NC}"
else
  echo -e "${RED}Redis 인스턴스 생성 실패 또는 이미 존재합니다.${NC}"
  echo "기존 Redis 연결 정보를 확인하세요: flyctl redis show ttsg-wiki-redis"
fi
...
echo "flyctl postgres show ttsg-wiki-db"

# Redis 연결 정보 가져오기
echo -e "\n${YELLOW}Redis 연결 정보 가져오는 방법:${NC}"
echo "flyctl redis show ttsg-wiki-redis"
