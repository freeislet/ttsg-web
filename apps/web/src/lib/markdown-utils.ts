  ...
  wikiLinks.forEach((link) => {
    ...
    link.addEventListener('mouseenter', (e) => {
      hoverTimeout = setTimeout(async () => {
        ...
        // 팝업 크기 추정 (최대 너비 384px = max-w-sm)
        const popupWidth = 384
        const popupHeight = 200 // 대략적인 높이
        ...
        // 오른쪽 경계 체크
        else if (left + popupWidth / 2 > viewport.width - 16) {
          left = viewport.width - 16
          transformX = '-100%'
        }
        ...
        // 하단 경계 체크 - 팝업이 화면 아래로 벗어나면 위쪽에 표시
        ...
        previewElement.style.transform = `translate(${transformX}, ${transformY})`
