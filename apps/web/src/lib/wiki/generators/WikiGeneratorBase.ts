...
export abstract class WikiGeneratorBase implements IWikiGenerator {
   ...
   * @returns 생성된 위키 콘텐츠
   */
  async generate(topic: string, language?: Language): Promise<WikiContent> {
    try {
      return await this._generate(topic, language ?? 'ko')
    } catch (error) {
   ...
   * @returns 생성된 위키 콘텐츠
   */
  abstract _generate(topic: string, language: Language): Promise<WikiContent>
}
